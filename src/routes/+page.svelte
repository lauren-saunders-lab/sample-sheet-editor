...
{#each rows as row, row_index (row)}
	{#each cols as col, col_index (col)}
		<Cell type={array[row_index][col_index]} selecting={selection_array[row_index][col_index]} onmouseenter={() => mouseenter(row_index, col_index)} onmousedown={() => mousedown(row_index, col_index)} onmouseup={() => mouseup(row_index, col_index)}>{row}{col}</Cell>
	{/each}
{/each}
</div>
